...
docker-ubuntu1404: depends scratch
	sudo /usr/share/docker.io/contrib/mkimage.sh -t cbayle/ubuntu:14.04 debootstrap --include=ubuntu-minimal --components=main,universe trusty http://archive.ubuntu.com/ubuntu
...
run8:
	docker run --rm=true -t -i cbayle/ubuntu:14.04 /bin/bash
